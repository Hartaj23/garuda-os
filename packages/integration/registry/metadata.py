...
class IntegrationRegistryMetadata:
...
def validate_integration_registry_metadata(
    metadata: object,
    field_prefix: str = "registry_metadata",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(metadata, IntegrationRegistryMetadata):
        result.add_error(
            "Registry metadata must be an IntegrationRegistryMetadata value.",
