...
class IntegrationLifecycleState(StrEnum):
    DRAFT = "draft"
    ACTIVE = "active"
    SUSPENDED = "suspended"
    CLOSED = "closed"
...
class IntegrationLifecycleMetadata:
...
def validate_integration_lifecycle_metadata(
    metadata: object,
    field_prefix: str = "lifecycle_metadata",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(metadata, IntegrationLifecycleMetadata):
        result.add_error(
...
def validate_integration_lifecycle_state(
    lifecycle_state: object,
    field_prefix: str = "lifecycle_state",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(lifecycle_state, IntegrationLifecycleState):
        result.add_error(
