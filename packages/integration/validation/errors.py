...
class IntegrationValidationIssue:
    ...
    field: str
    message: str
    issue_metadata: IntegrationValidationMetadata | None = None

    def __post_init__(self) -> None:
        if self.issue_metadata is None:
            object.__setattr__(self, "issue_metadata", IntegrationValidationMetadata())

    def to_dict(self) -> dict[str, object]:
        issue_metadata = self.issue_metadata or IntegrationValidationMetadata()
        return {
            "field": self.field,
...
@dataclass(frozen=True, slots=True)
class IntegrationValidationIssueCollection:
    issues: tuple[IntegrationValidationIssue, ...] = ()
...
def validate_integration_validation_issue(
    issue: object,
    field_prefix: str = "validation_issue",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(issue, IntegrationValidationIssue):
        result.add_error(
            "Validation issue must be an IntegrationValidationIssue value.",
