...
class InterfaceRegistryMetadata:
...
def validate_interface_registry_metadata(
    metadata: object,
    field_prefix: str = "registry_metadata",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(metadata, InterfaceRegistryMetadata):
        result.add_error(
            "Registry metadata must be an InterfaceRegistryMetadata value.",
