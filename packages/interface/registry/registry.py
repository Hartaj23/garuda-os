...
class InterfaceRegistry:
    """Process-local deterministic catalog of Interface Foundation registrations.

    Registry operations describe registered Interface Foundation artifacts only. They have
    ...
    def __init__(self) -> None:
        self._entries: dict[str, InterfaceRegistryEntry] = {}

    def register(self, entry: InterfaceRegistryEntry) -> None:
        validation = validate_interface_registry_entry(entry)
    ...
    def lookup_exact(self, registration_identifier: str) -> InterfaceRegistryEntry | None:
        return self._entries.get(registration_identifier)

    def lookup_by_artifact_type(
        self,
        artifact_object_type: str,
    ) -> tuple[InterfaceRegistryEntry, ...]:
        matches = [
            entry
            for entry in self._entries.values()
            if entry.adapter_descriptor.artifact_object_type == artifact_object_type
        ]
        return tuple(sorted(matches, key=lambda item: item.registration_identifier))

    def lookup(self, criteria: InterfaceRegistryLookupCriteria) -> InterfaceRegistryLookupResult:
        ...
        if criteria.artifact_object_type is not None:
            entries = [
                ...
                if entry.adapter_descriptor.artifact_object_type == criteria.artifact_object_type
            ]

        if criteria.adapter_identifier is not None:
            entries = [
                entry
                for entry in entries
                if entry.adapter_descriptor.adapter_identifier == criteria.adapter_identifier
            ]

        sorted_entries = tuple(sorted(entries, key=lambda item: item.registration_identifier))
        return InterfaceRegistryLookupResult(entries=sorted_entries)
    ...
    def validate(self) -> ValidationResult:
        ...
        for registration_identifier, entry in self._entries.items():
            ...
            result.merge(validate_interface_registry_entry(entry))
