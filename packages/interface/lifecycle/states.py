...
class InterfaceLifecycleState(StrEnum):
    DRAFT = "draft"
    ACTIVE = "active"
    SUSPENDED = "suspended"
    CLOSED = "closed"
...
class InterfaceLifecycleMetadata:
...
def validate_interface_lifecycle_metadata(
    metadata: object,
    field_prefix: str = "lifecycle_metadata",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(metadata, InterfaceLifecycleMetadata):
        result.add_error(
...
def validate_interface_lifecycle_state(
    lifecycle_state: object,
    field_prefix: str = "lifecycle_state",
) -> ValidationResult:
    result = ValidationResult()

    if not isinstance(lifecycle_state, InterfaceLifecycleState):
        result.add_error(
